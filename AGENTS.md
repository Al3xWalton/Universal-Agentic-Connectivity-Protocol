# AGENTS.md — Executor coordination for Codex / OpenAI agents on UACP

> **Cross-agent note.** UACP is developed by two AI coding agents that may operate on this repository: Codex / OpenAI agents (this file) and Claude Code (`CLAUDE.md` at the same path). The two files are kept tightly synchronized — when you make any architectural change documented here, mirror it to `CLAUDE.md`, and vice versa. The shared coordination primitives are `docs/memory/CURRENT.md` (living state), `docs/memory/CHANGELOG.md` (append-only session log), and `docs/open-questions.md` (UACP-specific deferred decisions). Routing between Claude Code and Codex is governed by `ORCHESTRATION.md` at this repo's root.
...
Two agent instances (or a Claude Code window and a Codex window) may run concurrently against this repo. The memory system is what makes that work:
...
- The human is the tiebreaker. If an agent sees a `CURRENT.md` that disagrees with what Alexander just said in chat, trust the human and update `CURRENT.md` immediately.
