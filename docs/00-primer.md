...
- **Wire format.** UACP artifacts are JSON documents ([[RFC8259](https://datatracker.ietf.org/doc/html/rfc8259)]) validated against a JSON Schema (Draft 2020-12). Each artifact references its schema version through a `$schema` URL of the form `https://uacp-protocol.org/schema/v1.x/uacp.schema.json` (specific URL form finalized in Stage 3).
